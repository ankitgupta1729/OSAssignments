...
int main(int argc,char *argv[])
{
	...
	if(fp==NULL)
		...
		{
			//while(fgets(read_data,1000000,fp) != NULL )
			  // printf("%s",read_data);
			//different  //while(line--){
               //fscanf(fp,"%s",&read_data[i]);
    ...
    printf("*****"); 
	int no_of_pages;//=read_data[0]-'0';
	//printf("%d\n",no_of_pages);	
	int no_of_frames;//=read_data[1]-'0';
	//printf("%d\n",no_of_frames);
	//line=no_of_pages;
    /*while(line--){
               fscanf(fp,"%s",&read_data[i]);
    ...
    /*for(i=0;i<no_of_pages;i++,j++)
    {
    	ref_str[i]=read_data[j]-'0';
    }
    for(i=0;i<no_of_pages;i++)
    	printf("%d\n",ref_str[i]);*/
    //printf("**");
